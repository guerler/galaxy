...
def test_resolve_structured_request_payload_gates_on_validated():
    # Every captured request — async API or workflow tool step — lives on
    # ``ToolRequest`` reached via ``Job.tool_request`` (or
    # ``ImplicitCollectionJobs.structured_request``). The resolver returns
    # the payload only when ``request_state == 'validated'`` and ``None``
    # otherwise (caller degrades to the legacy state walk).
    payload = {"input1": {"src": "hda", "id": 9}}

    validated = SimpleNamespace(id=5, request_state="validated", request=payload)
    assert workflow_request_state.resolve_structured_request_payload(
        job=SimpleNamespace(tool_request=validated)
    ) == payload

    failed = SimpleNamespace(id=5, request_state="validation_failed", request=payload)
    assert workflow_request_state.resolve_structured_request_payload(
        job=SimpleNamespace(tool_request=failed)
    ) is None

    not_validated = SimpleNamespace(id=5, request_state="not_validated", request=payload)
    assert workflow_request_state.resolve_structured_request_payload(
        job=SimpleNamespace(tool_request=not_validated)
    ) is None

    assert workflow_request_state.resolve_structured_request_payload(
        job=SimpleNamespace(tool_request=None)
    ) is None

    # ICJ source (mapped over): the same ToolRequest is reached via
    # ``ImplicitCollectionJobs.structured_request``.
    assert workflow_request_state.resolve_structured_request_payload(
        icj=SimpleNamespace(structured_request=validated)
    ) == payload
